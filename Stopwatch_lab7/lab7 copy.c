...
volatile unsigned char ten=0, one=0, tenth=0;   // time variables
volatile unsigned char ten_b, one_b, tenth_b, ten1, one1, tenth1;
...
/*
        ...
        if (state == 0) {           // state = 0, display 00.0
            //TCCR1B &= ~(1<<CS12);
            if ((ten_b!=ten) || (one_b!=one) || (tenth_b!=tenth)) {
                snprintf(display, 5, "%d%d.%d", ten, one, tenth);
                moveto(0x80);
                stringout(display);
            }
            
        } else if (state == 1) {    // state = 1, count
            TCCR1B |= (1<<CS12);    // set prescalar 256 and start counter
            snprintf(display, 5, "%d%d.%d", ten, one, tenth);
            moveto(0x80);
            stringout(display);
            
        } else if (state == 2) {                    // state = 2, freeze screen but internal timer cont
            //TCCR1B &= ~(1<<CS12);
            //ten=1; one=2; tenth=3;
            if ((ten_b!=ten) || (one_b!=one) || (tenth_b!=tenth)) {
                snprintf(display, 5, "%d%d.%d", ten1, one1, tenth1);
                moveto(0x80);
                stringout(display);
            }
        }
        ten_b=ten; one_b=one; tenth_b=tenth;
...
ISR (ADC_vect) { // update state 
    ...
    if(adcIn != 255) {      // if some button is pressed
        if ((adcIn > 45) && (adcIn < 60)) { // if UP (Start_Stop)
            ...
            if (state == 1) {       // if S1, change to S2
                state = 2;
                TCCR1B &= ~(1<<CS12);
                ten1=ten; one1=one; tenth1=tenth;
                TCCR1B |= (1<<CS12);
            }
            if (state == 2) {       // if S2, change to S1
                state = 1;
            }
        } 
    } 
